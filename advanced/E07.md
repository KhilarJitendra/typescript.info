...
## Table of Contents
1. [What are Template Literal Types?](#what-are-template-literal-types)
2. [Basic String Interpolation](#basic-string-interpolation)
3. [Union Cross-Multiplication](#union-cross-multiplication)
4. [Advanced Type Inference](#advanced-type-inference)
5. [Intrinsic String Manipulation](#intrinsic-string-manipulation)
...
type RoutePattern = `${HttpMethod} /${Resource}` | `${HttpMethod} /${Resource}/:id`;
...
function validateField<K extends keyof FormData>(
  ...
  const validatorName = `validate${field.charAt(0).toUpperCase() + field.slice(1)}` as keyof ValidationFunctions;
...
const cli: CLICommands = {
  ...
  runTest: (options) => console.log(`Testing with coverage: ${options.coverage}`),
